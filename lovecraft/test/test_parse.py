...
def test_gather():
    ...
    result = lovecraft.parse.gather_posts('test/posts')
    assert len(result) == 5, 'gather did not collect correct number of posts, expected 5, got %s' % len(result)


def test_parse_meta():
    result = lovecraft.parse.meta(post)
    assert result['category'] == 'Testing', 'Meta was not extracted'
